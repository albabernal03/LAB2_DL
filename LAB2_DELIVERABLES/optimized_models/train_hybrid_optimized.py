...
# Define OPTIMIZED parameter grid (reduced for practical execution)
param_grid = {
    'cnn_filters_1': [64, 128],                   # 2 options (reduced from 3)
    'cnn_filters_2': [128, 256],                  # 2 options (reduced from 3)
    'kernel_size': [3, 5],                        # 2 options (same)
    'lstm_units_1': [64, 128],                    # 2 options (reduced from 3)
    'lstm_units_2': [32, 64],                     # 2 options (reduced from 3)
    'dropout_cnn': [0.2, 0.3],                    # 2 options (same)
    'dropout_lstm': [0.3, 0.4],                   # 2 options (same)
    'dropout_dense': [0.2, 0.3],                  # 2 options (same)
    'learning_rate': [0.001],                     # 1 option (reduced from 3)
    'l2_lambda': [0.0, 0.001],                    # 2 options (reduced from 3)
    'use_batch_norm': [True],                     # 1 option (reduced from 2)
    'bidirectional': [True],                      # 1 option (reduced from 2)
    'batch_size': [4, 8]                          # 2 options (same)
}

# Total: 2 × 2 × 2 × 2 × 2 × 2 × 2 × 2 × 1 × 2 × 1 × 1 × 2 = 512 combinations (~2-4 hours)
